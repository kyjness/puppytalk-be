...
- [x] 작업 큐를 **Celery → ARQ** — 0018이 ARQ를 기각한 사유 셋 중 둘이 성립하지 않았다(재시도 기능
      있음·큐 라우팅은 잡이 하나라 나눌 대상 없음). 동기↔async 어댑터 105줄과 요청 경로
      `run_in_threadpool` 왕복 제거. 리뷰에서 결함 3건 추가 발견(명령 타임아웃 부재·설정 이름
      무언의 변경·재시도 백오프 하한 누락) ([ADR 0020](adr/0020-worker-queue-arq.md))
- [ ] **#37** 실시간 전달 심화 — 큐 기반 소켓별 전달 (P2 · 이연. 연결 상한은 #42로 선반영)
- [ ] `enqueue_job`에 `_job_id` 전달 — 왕복 3회 중 2회가 무의미(존재할 수 없는 랜덤 키 조회).
      결정적 키를 넘기면 큐에서 중복 제거되지만 제거 지점이 워커→큐로 앞당겨져 동작이 바뀐다
