...
> **수정 완료(comments 도메인, 2단계)**:
> ① 1차(`ca8816ed`) — 500건 하드 리밋과 인메모리 슬라이스를 제거하고 루트 keyset + 대댓글 배치 로드로 트리를 조립. 대댓글 상한 부재는 #21에서 윈도우 함수로 루트당 상위 N건 제한.
> ② 2차(`480a2ba0`, [ADR 0016](adr/0016-comment-list-offset-pagination.md)) — 기본 정렬을 인기순으로 되돌리면서 **루트 목록을 offset + `total`로 전환**했다. 정렬 축 `like_count`가 변동값이라 keyset이 성립하지 않고, 댓글은 게시글 1건에 국한된 유한 집합이라 deep-offset이 실질 비용이 아니다. 대댓글은 축(`parent_id`·`id`)이 불변이라 **커서를 유지**한다.
> ⚠️ 따라서 [`01-architecture.md`](01-architecture.md) C2의 "루트 keyset + 대댓글 부모별 로드"는 현재와 **루트/대댓글이 반대**다 — 해당 문서에 개정 노트를 달아 뒀다.
