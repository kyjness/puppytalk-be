...
1. **전송 선택 = 방향성에 맞춤**
   ...
   - **종료 코드 계약**: `1008`=인증·권한 실패(`ws_close_code`의 4xx), `4001`=유저당 동시 연결
     ...
     (매니저 send 타임아웃 — 재접속 유도, 서버 오류 아님), `1011`=내부 오류(5xx). 용량 사유를 1008에서
     갈라낸 이유는 **복구 동작이 다르기 때문**이다 — 1008은 재인증, 4001은 재연결 포기,
     4002는 백오프 재연결. 한 코드로 뭉치면 클라이언트가 구분할 수 없어, 탭을 여러 개 열거나
     빠르게 도배한 것뿐인데도 로그인 세션을 폐기하는 오작동이 난다(실제로 그랬다).
...
2. **멀티 인스턴스 fanout = Redis Pub/Sub, 채팅·알림 공용 패턴**
   - **단일 채널 + envelope `{origin, target_user_ids, payload}`** — 채팅
     ...
     건별 발행은 발행 RTT와 전 인스턴스 파싱을 수신자 수만큼 증폭한다. 파서·발행자 모두
     `target_user_ids` 하나만 다룬다 — 형식이 하나뿐이므로 롤링 배포 창에서 구/신 인스턴스가
     섞여도 계약이 갈리지 않는다.
