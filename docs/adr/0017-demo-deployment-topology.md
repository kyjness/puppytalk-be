...
## 운영 봉투와의 관계 (Relation to the Operating Envelope)

이 배포는 [운영 봉투](../00-operating-envelope-and-scope.md)를 **만족하지 않는다.** 이탈하는 축은 이렇다.

| 봉투의 축 | 전제 | 이 배포 | |
|---|---|---|---|
| 규모 | 평상시 수천~수만 DAU | 2GB 인스턴스 한 대 | **이탈** |
| 서버 | 멀티 인스턴스 3~10대 | 1대 | **이탈** |
| 배포 | 무중단 롤링 / 블루-그린 | `compose up -d`, 수 초 끊김 | **이탈** |
| 가용성 | 99.9% (월 ~43분) | 재부팅 한 번이면 예산 소진 | **이탈** |
| 핫스팟 | 인기글 1건 초당 수백~수천 조회 | 프록시는 여유, 뒤가 못 버팀 | 부분 |

봉투를 만족하는 설계는 `puppytalk-infra/terraform/`(ALB · ECS Fargate · RDS Multi-AZ ·
ElastiCache)이다. **이 데모는 그 설계의 구현이 아니라, 같은 앱을 쇼케이스 등급으로 축소한 별개
트랙이다.**

이탈한 것은 배포 등급이지 앱이 아니다 — 봉투가 앱에 건 "상태는 인스턴스 밖, 로컬 상태 금지"는
그대로 지킨다(DM은 워커 간 Redis pub/sub, [ADR 0009](0009-realtime-delivery.md)). 인스턴스를
3~10대로 늘릴 때 바뀌는 것은 앞단 LB와 `.env`의 신뢰 프록시 대역뿐이고, 그 시점에 Caddy 자리는
**ALB로 교체된다** — 처리량이 모자라서가 아니라 Caddy가 도는 인스턴스 자신이 SPOF라서다.
...
## 배포 후 검증은 데모 계정으로만 한다

**운영에서 새 계정을 만들지 않는다.** 이 사이트는 로그인 없이도 피드·게시글이 보이고(가시성은
`current_user_id`와 무관하다), 회원가입으로 만든 계정과 그 글은 **방문자에게 그대로 노출된다**.
검증하겠다고 만든 `smoke*` 계정이 포트폴리오 첫 화면에 남는다는 뜻이다.

검증 절차는 이 순서로 충분하고, 행을 하나도 만들지 않는다:

1. `/livez`·`/readyz` — 컨테이너·DB·Redis 기동 확인
2. 로그인 없이 피드·게시글 상세가 뜨는지 — 익명 조회 경로
3. **데모 계정으로 로그인**(`demo@puppytalk.shop`) — 이미 있는 계정을 쓴다
4. 쓰기까지 볼 거면 **그 데모 계정으로** 글·댓글·좋아요·DM을 남긴다

4에서 남긴 것은 `seed-demo --force`의 정리 범위 안에 있다(`purge_demo_data`가 `DEMO_USERS`
소유 콘텐츠를 지운다) — **검증을 데모 계정에 가두면 기존 도구가 곧 정리 경로**이므로, 별도의
청소 명령이 필요 없다.

> **"데모 계정을 제외한 전부"를 지우는 명령은 만들지 않는다.** 지울 대상이 아니라 남길 대상을
> 나열하는 방식이라, 운영에서 한 번 잘못 돌면 실제 회원이 사라진다. 얻는 것은 수동 정리 몇 분뿐이다.
> 또한 `posts.user_id`·`comments.author_id`의 `SET NULL`은 **탈퇴해도 글은 남긴다**는 정책
> (`UserService.purge_withdrawn_users`)이라, 유저를 콘텐츠째 쓸어내는 도구는 그 정책과 충돌한다.
> 훗날 정리 도구가 필요해지면 테스트 도메인(`@test.dev`·`@ex.com` 등)을 **열거하는** 방식으로
> 만든다 — 실수의 결과가 "아무것도 안 지워짐"이 되도록.
