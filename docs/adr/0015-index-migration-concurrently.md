...
- **이 규약 이전 리비전** — 소급 적용하지 않는다. 예: `caabca5fb0ad_add_idx_posts_feed_latest`
  (2026-04-01, `003` 직후)는 `posts`에 비-`CONCURRENTLY` 인덱스를 만든다. 아래 판정 기준으로는
  위반이지만 이 ADR보다 먼저 작성됐고(맥락 절의 "한 번도 쓰이지 않았다"가 그 상태다),
  **이미 적용된 마이그레이션을 사후 수정하면 alembic이 재실행하지 않아 환경마다 스키마
  이력이 갈린다** — 고치는 쪽이 더 위험하다. 규약은 신규 리비전부터 적용한다.
