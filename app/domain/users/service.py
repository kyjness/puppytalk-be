...
class UserService:
    ...
    async def check_availability(
        ...
        async with db.begin():
            return AvailabilityData(
                email_available=not await UsersRepository.email_exists(query.email, db=db)
                if query.email
                else None,
                nickname_available=not await UsersRepository.nickname_exists(query.nickname, db=db)
                if query.nickname
                else None,
            )
    ...
    async def get_user_profile(cls, user_id: UUID, db: AsyncSession) -> UserProfileResponse:
        async with db.begin():
            user_with_dogs = await UsersRepository.get_user_by_id_with_dogs(user_id, db=db)
    ...
    async def update_user_profile(
        ...
        async with db.begin():
            ...
            if data.nickname is not None:
                if data.nickname != user.nickname and await UsersRepository.nickname_exists(
                    data.nickname, db=db
                ):
            ...
            # 2) 유저 행 갱신
            if updates and not await UsersRepository.update_user(user_id, db=db, **updates):
            ...
            db.expire(user)
            user_updated = await UsersRepository.get_user_by_id_with_dogs(user_id, db=db)
            if not user_updated:
                raise InternalServerErrorException()
    ...
    async def update_password(
        ...
        async with db.begin():
            hashed = await UsersRepository.get_password_hash(user_id, db=db)
            ...
            if not await UsersRepository.update_password(user_id, new_hashed, db=db):
                raise InternalServerErrorException()
    ...
    async def toggle_block_user(
        ...
        async with db.begin():
            exists = await UsersRepository.block_exists(blocker_id, target_user_id, db=db)
            if exists:
                await UsersRepository.unblock_user(blocker_id, target_user_id, db=db)
                return False
            await UsersRepository.block_user(blocker_id, target_user_id, db=db)
            return True
    ...
    async def delete_user(cls, user_id: UUID, db: AsyncSession) -> None:
        async with db.begin():
            ...
            if not user:
                raise InternalServerErrorException()
            if not await UsersRepository.delete_user(user_id, db=db):
                raise InternalServerErrorException()
